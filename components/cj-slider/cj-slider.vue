<template>
  <view class="cj-slider" @tap="onClick" :class="[disabled ? 'cj-slider--disabled' : '']" :style="{
    ...
    <view class="cj-slider__gap" :style="[
      ...
      <view class="cj-slider__button-wrap" @touchstart="onTouchStart(1, $event)" @touchmove="onTouchMove(1, $event)"
        @touchend="onTouchEnd(1)" @touchcancel="onTouchEnd">
      ...
      <view class="cj-slider__button-wrap2" @touchstart="onTouchStart(2, $event)" @touchmove="onTouchMove(2, $event)"
        @touchend="onTouchEnd(2)" @touchcancel="onTouchEnd">
        <slot v-if="$slots.default  || $slots.$default" />
        <view v-else class="cj-slider__button" :style="[blockStyle, {
      				height: blockWidth + 'rpx',
      				width: blockWidth + 'rpx',
  ...
  export default {
    ...
    watch: {
      value(n) {
        ...
        if (this.status == 'end') {
          this.updateValue(n[0], n[1], false);
        }
      },
    },
    created() {
    ...
    methods: {
      ...
      onClick(event) {
        ...
        const value = this.format(((event.detail.x - this.sliderRect.left) / this.sliderRect.width) * (this.max - this.min) +
          this.min);
